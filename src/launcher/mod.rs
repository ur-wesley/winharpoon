...
static PENDING_LAUNCHER: AtomicBool = AtomicBool::new(false);
static PENDING_SETTINGS: AtomicBool = AtomicBool::new(false);
...
pub fn init(config: Arc<Mutex<Config>>, state: Arc<Mutex<AppState>>, marks: SharedMarks) {
    INIT.call_once(|| {
        ...
        thread::spawn(move || ui::run_ui(config, state, marks, rx));
...
pub(crate) fn take_pending_launcher() -> bool {
    PENDING_LAUNCHER.swap(false, Ordering::AcqRel)
}

pub(crate) fn take_pending_settings() -> bool {
    PENDING_SETTINGS.swap(false, Ordering::AcqRel)
}
...
fn signal(command: UiCommand) {
    match command {
        UiCommand::Launcher => PENDING_LAUNCHER.store(true, Ordering::Release),
        UiCommand::Settings => PENDING_SETTINGS.store(true, Ordering::Release),
        UiCommand::TrayMenu => {}
    }
