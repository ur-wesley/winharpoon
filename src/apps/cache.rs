...
pub fn load_or_scan() -> Vec<AppEntry> {
    let roots = enumerate::start_menu_roots();
    let mtime = enumerate::dir_mtime(&roots);
    let path = paths::apps_cache_path();

    if path.exists() {
        if let Ok(text) = fs::read_to_string(&path) {
            if let Ok(cache) = toml::from_str::<AppsCacheFile>(&text) {
                if cache.mtime == mtime && !cache.entries.is_empty() {
    ...
    save_cache(&path, mtime, &entries);
    entries
}

pub fn save_cache(path: &Path, mtime: u64, entries: &[AppEntry]) {
    #[cfg(test)]
    {
        let _ = (path, mtime, entries);
    }
    #[cfg(not(test))]
    {
